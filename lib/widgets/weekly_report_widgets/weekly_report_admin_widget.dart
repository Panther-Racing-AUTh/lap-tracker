...
class _WeeklyReportAdminWidgetState extends State<WeeklyReportAdminWidget> {
  ...
  Future<void> _loadUserList() async {
    try {
      ...
      List<dynamic> userList = await getUserList();
      ...
      setState(() {
        // Perform UI updates or additional logic here
      });
    } catch (e) {
                          ...
                          child:Column(
                            ...
                            children: [
                              for (int i=0;i<weeklyReportProvider.reports.length;i++)
                                GestureDetector(
                                    onTap: () async{
                                      var report_data=await getWeeklyReportsFromUser(weeklyReportProvider.reports[i].userId);
                                      weeklyReportProvider.reports[i].reportList.clear();
                                      for(int j=0;j<report_data.length;j++){
                                        weeklyReportProvider.reports[i].reportList.add(WeeklyReportListItemDate.fromMap(report_data[j]));
                                        print(WeeklyReportListItemDate.fromMap(report_data[j]).userId);
                                      }
  ...
  return Container(
    ...
    child: Row(
      children: [
        ...
        Spacer(),
        Icon(item.weeklyReportSubmitted ? Icons.circle : Icons.circle_outlined)
      ],
    ),
  );
}
