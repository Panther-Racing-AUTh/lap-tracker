...
Map<String, dynamic> ExpenseToMap(Appointment appointment) {
  return {
...
Future<void> saveAppointment(Appointment appointment) async {
  try {
    final response = await supabase.from('appointment').insert(ExpenseToMap(appointment)).execute();
...
Future<void> saveAllAppointments(List<Appointment> appointmentList) async {
  for(int i=0;i<appointmentList.length;i++){
    try {
      await supabase.from('appointment').insert(ExpenseToMap(appointmentList[i])).execute();
...
Future<void> updateAppointment(Appointment appointment) async {
  try{
    final response = await supabase
        .from('appointment')
        .update(ExpenseToMap(appointment))
...
Future<void> insertAppointment(Appointment appointment) async {
  final response = await supabase
      .from('appointment')
      .insert(ExpenseToMap(appointment))
      .execute();
